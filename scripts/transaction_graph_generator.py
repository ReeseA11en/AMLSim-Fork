...
class TransactionGenerator:
    ...
    def generate_normal_transactions(self):
        ...
        nodes = list(self.g.nodes()) # Changed to list to fix array adressing
        # print(list(self.g.nodes(data=True))) # Prints all nodes with data
    ...
    def add_account(self, acct_id, **attr):
        ...
        for key in attr.keys():
            self.g.nodes[acct_id][key] = attr[key]
        
        #print(self.g.nodes[0])
        #print(attr)
        # nx.set_node_attributes(self.g.nodes[acct_id],attr)
        # self.g.nodes[acct_id] = attr
    ...
    def add_edge_info(self, orig, bene):
        ...
        self.g[orig][bene]["edge_id"] = self.edge_id 
        self.edge_id += 1
    ...
    def add_aml_typology(self, is_sar, typology_name, num_accounts, min_amount, max_amount, period, bank_id="", schedule=1):
        ...
        def add_node(_acct, _bank_id):
            ...
            sub_g.add_node(_acct)
            for key in attr_dict.keys():
                sub_g.nodes[_acct][key] = attr_dict[key]
            #sub_g.add_nodes_from([_acct], attr_dict)
        ...
        def add_main_acct():
            ...
            """
            self.check_hub_exists()
            _main_acct = random.sample(sorted(self.hubs), 1)[0] # sorted() self.hubs to follow error recommendation
        ...
        if typology_name == "fan_in":  # fan_in pattern (multiple accounts --> single (main) account)
            ...
            if is_external:
                if len(self.get_all_bank_ids()) >= 3:
                    [orig_bank_id, mid_bank_id, bene_bank_id] = random.sample(sorted(self.get_all_bank_ids()), 3)
                else:
                    [orig_bank_id, mid_bank_id] = random.sample(sorted(self.get_all_bank_ids()), 2)
                    bene_bank_id = orig_bank_id
            else:
                orig_bank_id = mid_bank_id = bene_bank_id = random.sample(sorted(self.get_all_bank_ids()), 1)[0]
        ...
        with open(alert_member_file, "w") as wf:
            ...
            for gid, sub_g in self.alert_groups.items():
                ...
                for n in sub_g.nodes():
                    ...
                    if get_out_edge_attrs(sub_g, n, "amount") == []: # Adding to avoid errors with an empty list
                        min_amt = 100 # Setting the default minimum to match that of all alert groups
                        max_amt = 400
                    else:
